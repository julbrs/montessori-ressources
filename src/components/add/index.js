...
var Add = () => {
  ...
  const uploadFiles = async () => {
    ...
    Promise.all(promises).then(() => {
      ...
      //send `POST` request
      // client
      //   .post("/nomenclatures", fd, axiosConfig)
      //   .then(() => {
      //     setUploading(false);
      //     setSuccess(true);
      //   })
  ...
  return (
    <Container style={{ marginBottom: "30px" }}>
      <Heading size={5}>
        Vous pouvez participer à ce projet en fournissant vos propres images
        pour faire des nomenclatures !
      </Heading>
      <Heading subtitle size={5}>
        Utilisez simplement le formulaire ci-dessous pour déposer vos images,
        notre équipe se chargera de valider le contenu avant de le proposer sur
        notre outil. Votre nom sera crédité sur la nomenclature.
      </Heading>
      <Box {...getRootProps()}>
        <input {...getInputProps()} />
        {isDragActive ? (
